...
function filterOrders(orders, query) {
  ...
  return orders.filter(
    (o) =>
      ...
      String(o.storeName || '').toLowerCase().includes(q) ||
...
function OrderDetailRow({ order, status }) {
  ...
  const itemsWithOptions = (order.items || [])
    .map((it) => {
      const opts = (it.options || it.modifiers || []).length
        ? '<br><small>' + (it.options || it.modifiers || []).map((m) => '  ' + m).join('<br>') + '</small>'
        : '';
  ...
  const totalStrDetail = order.total != null ? `$${Number(order.total).toFixed(2)}` : (p.total || '—');
      ...
      <td colSpan={11}>
        <div className="detail-grid">
          <div className="detail-section">
            ...
            {cardAuth ? <><br />{cardAuth}</> : null}
            {p.barcode ? <><br />Barcode: {p.barcode}</> : null}
            <br />
            {receiptFooter}
          </div>
          <div className="detail-section">
...
function OrderRow({ order, onShowJson, onRefresh, confirmDialog, addToast, api }) {
  ...
  const fulfillment = (order.fulfillmentType || '').replace(/^\w/, (c) => c.toUpperCase());
  const payment = (order.paymentMethod || '').replace(/^\w/, (c) => c.toUpperCase());
  const itemCount = order.itemCount != null ? order.itemCount : (Array.isArray(order.items) ? order.items.length : 0);
  ...
  return (
    <>
      ...
      >
        ...
        <td>
          <span className={`status-pill ${statusClass}`} title={status === 'failed' && order.printError ? order.printError : statusLabel}>
            {statusLabel}
          </span>
        </td>
        <td>
...
export default function QueueTable({ orders, onShowOrderJson, onRefresh, confirmDialog, addToast, api }) {
  ...
  return (
    <section className="section-card queue-section">
      <div className="queue-section-header">
        <h2>Print queue</h2>
        <span className="queue-count">{filtered.length} order{filtered.length !== 1 ? 's' : ''}</span>
      </div>
      ...
      {(!orders || orders.length === 0) && (
        <div className="queue-empty">
          <div className="queue-empty-icon" aria-hidden>📋</div>
